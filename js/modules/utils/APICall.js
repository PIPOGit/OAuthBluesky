...
export async function tryAndCatch( currentStep, callbackFunction, callbackOptions=null, show=true ) {
	...
	const step							= callbackFunction.name;
	if (show && window.BSKY.GROUP_DEBUG) console.groupCollapsed( PREFIX + `Step: ${step}` );
	...
	try {
		...
		if ( error.hasOwnProperty("step") ) {
			...
			let sameSteps				= ( error?.step ) ? COMMON.areEquals(error.step, step) : false;
			let distinctDPoPNonce		= (BSKY.data?.dpopNonceUsed && BSKY.data?.dpopNonceReceived) ? !COMMON.areEquals(BSKY.data?.dpopNonceUsed, BSKY.data?.dpopNonceReceived) : false;
			...
			if ( sameSteps && distinctDPoPNonce) {
				...
				if (show && window.BSKY.DEBUG) console.debug( PREFIX_RETRY + `+ [step]`, step );
