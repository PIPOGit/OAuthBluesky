 ...
 **********************************************************/
// PreLoaded constants
// -- AVAILABLES @ ./modules/common.functions.js
...
// Bluesky constants
const URL_LOCALHOST					= CONFIGURATION.localhost.root;
const APP_CLIENT_ID					= NEOCITIES.client_id;
const APP_CALLBACK_URL				= NEOCITIES.redirect_uri;
const URL_RESOLVE_HANDLE			= API.bluesky.XRPC.url + API.bluesky.XRPC.api.resolveHandle + "?handle=";
const URL_PLC_DIRECTORY				= API.plc.url;
const URL_PDS_METADATA				= API.pds.api.metadata;
const URL_AUTH_DISCOVERY			= API.authServer.api.discovery;
...
// -- AVAILABLES @ ./modules/OAuth2/JWT.js
const prettyStringJWTPart			= str => JSON.stringify(JSON.parse(atob(toBase64UrlString(str))), null, "    ");
// -- AVAILABLES @ ====================================================
 ...
 * HELPER Functions
 **********************************************************/
// -- AVAILABLES @ ./modules/OAuth2/Base64Url.js
...
// -- AVAILABLES @ ./modules/OAuth2/UUID.js
...
// -- AVAILABLES @ ./modules/OAuth2/PKCE.js
...
// -- AVAILABLES @ ./modules/OAuth2/Crypto.js
...
function getJWTAsString(accessToken) {
    ...
    var headerString = prettyStringJWTPart(parts.header);
    var payloadString = prettyStringJWTPart(parts.payload);
...
function processAPICallResponse(step, response) {
	...
	if ( response.ok ) {
		...
		return response.text().then( data => {
			const parsedResponse = printOutFetchResponse( response );
			const contentType = parsedResponse.headers["content-type"];
			let errorObject = new TYPES.APICallError( step, parsedResponse, contentType );
...
function step06RedirectUserToBlueskyAuthPage() {
	...
	localStorage.setItem(LSKEYS.request.authentication, JSON.stringify( userAuthentication ));
	localStorage.setItem(LSKEYS.request.access_token, userAccessToken);
	localStorage.setItem(LSKEYS.crypto.jwk, prettyJson( jwk ));
...
async function retrieveTheUserAccessToken(code) {
    ...
    let cryptoKey = await crypto.subtle.generateKey(cryptoKeyOptions, true, cryptoKeyPurposes).then(function(eckey) {
        return eckey;
    });
    ...
    delete jwk.ext;
    delete jwk.key_ops;
	if (DEBUG) console.debug( PREFIX, "jwk:", prettyJson( jwk ) );
	localStorage.setItem(LSKEYS.crypto.jwk, prettyJson( jwk ));
...
function fnAnalizeCallbackURL() {
	const PREFIX = `[${MODULE_NAME}:fnAnalizeCallbackURL] `;
	if (GROUP_DEBUG) console.groupCollapsed(PREFIX);
	...
	if (!areEquals(thisURL.hostname, "localhost")) {
		...
		if (DEBUG) console.debug(PREFIX, "Processing the request in localhost. Invoking: [fnProcessCallback]");
		if (GROUP_DEBUG) console.groupEnd();
		fnProcessCallback(parsedSearch);
	}
}
